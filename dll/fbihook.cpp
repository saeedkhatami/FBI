...
void parse_preferred_binding() {
    ...
    if (GetEnvironmentVariable(_T("VERBOSE"), buffer, 256) > 0) {
        ...
        if (verbose) _tprintf(_T("Verbose mode enabled in DLL\n"));
    }
    ...
    if (GetEnvironmentVariable(_T("PREFERRED_IP"), buffer, 256) > 0) {
        ...
        if (pref_ip.find(_T("IPv4:")) == 0) {
            ...
            if (InetPton(AF_INET, ip_str.c_str(), &g_preferred_ipv4_addr.sin_addr) == 1) {
                ...
                if (verbose) _tprintf(_T("Set preferred IPv4 address: %s\n"), ip_str.c_str());
            }
        }
            ...
            if (InetPton(AF_INET6, ip_str.c_str(), &g_preferred_ipv6_addr.sin6_addr) == 1) {
                ...
                if (verbose) _tprintf(_T("Set preferred IPv6 address: %s\n"), ip_str.c_str());
            }
        }
    }
        ...
        if (verbose) _tprintf(_T("Standardized GUID: %s\n"), guid_str.c_str());
        ...
        if (dwRetVal == NO_ERROR) {
            for (PIP_ADAPTER_ADDRESSES pAdapter = pAddresses; pAdapter; pAdapter = pAdapter->Next) {
                ...
                if (adapterName == guid_str) {
                    ...
                    if (verbose) _tprintf(_T("Set interface index: %lu\n"), g_interface_index);
...
static BOOL enforce_binding(SOCKET s, int family, const struct sockaddr* target) {
    ...
    if (family == AF_INET && g_preferred_ipv4_addr.sin_addr.s_addr != 0) {
        ...
        ((SOCKADDR_IN*)&bind_addr)->sin_addr.s_addr = g_preferred_ipv4_addr.sin_addr.s_addr;
        ...
        ((SOCKADDR_IN6*)&bind_addr)->sin6_addr = g_preferred_ipv6_addr.sin6_addr;
        bind_len = sizeof(SOCKADDR_IN6);
    }
    else {
        if (verbose) _tprintf(_T("No preferred address for family %d\n"), family);
        return TRUE; 
    }

    int bind_result = original_bind(s, (SOCKADDR*)&bind_addr, bind_len);
    if (bind_result != 0) {
        if (verbose) _tprintf(_T("Binding failed: %d\n"), WSAGetLastError());
        return FALSE;
    }
...
SOCKET WSAAPI hooked_socket(int af, int type, int protocol) {
    ...
    if (verbose) _tprintf(_T("Socket %d created: af=%d, type=%d, protocol=%d\n"), s, af, type, protocol);
    return s;
}
...
int WSAAPI hooked_connect(SOCKET s, const struct sockaddr* name, int namelen) {
    ...
    if (getsockopt(s, SOL_SOCKET, SO_TYPE, (char*)&sock_type, &optlen) == SOCKET_ERROR) {
        if (verbose) _tprintf(_T("Failed to get socket type: %d\n"), WSAGetLastError());
        return SOCKET_ERROR;
    }

    if (sock_type == SOCK_STREAM) {
        if (!enforce_binding(s, name->sa_family, name)) {
            if (verbose) _tprintf(_T("Failed to enforce binding for TCP socket %d\n"), s);
            return SOCKET_ERROR;
        }
    }

    int result = original_connect(s, name, namelen);
    if (verbose && result != 0) _tprintf(_T("Connect failed: %d\n"), WSAGetLastError());
    return result;
}
    ...
    if (getsockopt(s, SOL_SOCKET, SO_TYPE, (char*)&sock_type, &optlen) == 0 && sock_type == SOCK_DGRAM) {
        ...
        if (original_getsockname(s, (sockaddr*)&addr, &addr_len) == SOCKET_ERROR && WSAGetLastError() == WSAEINVAL) {
            if (!enforce_binding(s, to->sa_family, to)) {
                if (verbose) _tprintf(_T("Failed to enforce binding for UDP socket %d\n"), s);
                return SOCKET_ERROR;
            }
        }
    }
...
static BOOL initialize_hooks() {
    ...
    if (!original_socket || !original_bind || !original_connect || !original_sendto || !original_getsockname) return FALSE;
    ...
    if (verbose) _tprintf(_T("Hooks initialized successfully\n"));
    return TRUE;
}
